...
if (config.LANG == 'EN') {
  ...
  sqadir_desc = 'change Auto bio mode'
...
if (config.LANG == 'ML') {
  ...
  sqadir_desc = 'Auto bio mode മാറ്റാൻ'
  OFF = '*AUTO BIO OFF ആക്കി💌*'
  ON = '*AUTO BIO ON ആക്കി💌*'
}

SQADIR.addCommand({ pattern: 'autobio ?(.*)', fromMe: true, desc: sqadir_desc, usage: '.autobio on / off' }, (async (message, match) => {
  if (match[1] == 'off') {
